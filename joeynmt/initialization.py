...
def initialize_model(model: nn.Module, cfg: dict, src_padding_idx: int,
    ...
    with torch.no_grad():
        ...
        elif isinstance(model.src_embed, MultispaceEmbeddings):
            for src_embed in model.src_embed.values():
                src_embed.lut.weight.data[src_padding_idx].zero_()
        else:
            if isinstance(model.src_embed.query_emb, Embeddings):
                model.src_embed.query_emb.lut.weight.data[src_padding_idx].zero_()
            elif isinstance(model.src_embed.query_emb, MultispaceEmbeddings):
                for src_embed in model.src_embed.query_emb.values():
                    src_embed.lut.weight.data[src_padding_idx].zero_()
        if isinstance(model.trg_embed, Embeddings):
            model.trg_embed.lut.weight.data[trg_padding_idx].zero_()
        elif isinstance(model.trg_embed, MultispaceEmbeddings):
            for trg_embed in model.trg_embed.values():
                trg_embed.lut.weight.data[trg_padding_idx].zero_()
