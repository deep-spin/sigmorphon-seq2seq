...
class RecurrentEncoder(Encoder):
    ...
    def _check_shapes_input_forward(self, embed_src, src_length, mask):
        """
    ...
    def forward(self, embed_src: Tensor, src_length: Tensor, mask: Tensor) \
        ...
        :return:
            ...
            - attn: None (returned to keep same interface as Transformer)
        """
        self._check_shapes_input_forward(embed_src, src_length, mask)

        # apply dropout to the rnn input
        embed_src = self.emb_dropout(embed_src)

        packed = pack_padded_sequence(
            embed_src, src_length, batch_first=True,
            enforce_sorted=self._enforce_sorted)
...
class TransformerEncoder(Encoder):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            TransformerEncoderLayer(size=hidden_size, ff_size=ff_size,
                                    ...
                                    attn_func=attn_func, attn_alpha=attn_alpha)
    ...
    def forward(self,
                embed_src: Tensor,
                src_length: Tensor,
                mask: Tensor) -> (Tensor, Tensor, dict):
        """
        ...
        :return:
            - output: hidden states with
                shape (batch_size, max_length, directions*hidden),
            - hidden_concat: None
            - attn: dictionary containing src-src attention for the batch
        """
        ...
        attn = {"src_src": torch.stack(self_attn_layers, dim=1)}
        return self.layer_norm(x), None, attn
