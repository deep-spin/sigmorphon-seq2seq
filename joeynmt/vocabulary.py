...
def build_vocab(field: str, max_size: int, min_freq: int, dataset: Dataset,
                sequential: bool = True, vocab_file: str = None,
                multilingual: bool = False) -> Vocabulary:
    """
    ...
    if vocab_file is not None:
        ...
        tokens = [getattr(ex, field) for ex in dataset.examples]
        if sequential:
        ...
        vocab = Vocabulary(tokens=vocab_types)
        assert len(vocab) <= max_size + len(vocab.specials)
        assert vocab.itos[DEFAULT_UNK_ID] == UNK_TOKEN

    # check for all except for UNK token whether they are OOVs
    assert not any(vocab.is_unk(s) for s in vocab.specials[1:])

    language_counts = defaultdict(Counter)  # kinda perverse
    for ex in dataset.examples:
        language = ex.language if hasattr(ex, "language") else None
        line_toks = getattr(ex, field)
        language_counts[language].update(vocab.stoi[t] for t in line_toks)
    for language in language_counts:
        language_counts[language].update(vocab.stoi[s] for s in vocab.specials)

    return vocab, language_counts
