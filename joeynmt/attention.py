...
class MultiAttention(nn.Module):
    ...
    def __init__(self, attn_type: str, head_names: List[str],
                 key_sizes: Dict[str, int], hidden_size: int,
        ...
        self.heads = nn.ModuleDict(
            {k: attn(key_size=key_sizes[k], hidden_size=hidden_size,
    ...
    def forward(self, query: Tensor, srcs: Dict[str, Tensor]):
        assert all(name in self.head_names for name in srcs)

        contexts = dict()
        att_probs = dict()
        for head_name in self.head_names:
            values, mask = srcs[head_name]
            context, alphas = self.heads[head_name](
                query=query, mask=mask, values=values
            )
...
class AttentionConcat(nn.Module):
    def __init__(self, enc_sizes: dict, hidden_size, head_names: list):
        super(AttentionConcat, self).__init__()
        self.head_names = head_names
        input_size = hidden_size + sum(enc_sizes.values())
...
class AttentionGate(nn.Module):
    ...
    def __init__(self, enc_sizes: dict, hidden_size, head_names: list,
        ...
        gate_input_dim = sum(enc_sizes.values()) + hidden_size
        n_classes = len(enc_sizes)
        ...
        for name, enc_size in enc_sizes.items():
